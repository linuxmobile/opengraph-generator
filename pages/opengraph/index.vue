<template>
	<main class="relative h-screen flex flex-col-reverse p-5 overflow-auto transition-all"
		...
		<div class="relative h-screen w-full flex flex-col-reverse p-5 overflow-auto md:flex-col">
			<MetadataOptions v-show="isOptionsPanelOpen" v-bind="metadata" @metadata-update="updateMetadata"
				@toggle-menu="toggleMenu" />
			<div class="w-full flex items-center justify-between gap-x-5 pt-5">
				<button class="w-full rounded-lg py-2 bg-white/5" @click="generateShareableLink">Generate Shareable
					Link</button>
			...
			<SliderPreviewOg :title="metadata.title" :description="metadata.description" :author="metadata.author"
				:url="metadata.url" :svg="absoluteFaviconUrl" :selectedId="selectedId" @updateSelectedId="updateSelectedId" />
			<PreviewOG :title="metadata.title" :description="metadata.description" :author="metadata.author"
				:url="metadata.url" :svg="absoluteFaviconUrl" :selectedId="selectedId" @updateSelectedId="updateSelectedId"
