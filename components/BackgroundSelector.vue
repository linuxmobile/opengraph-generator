...
<template>
  <div class="flex flex-wrap gap-4">
    <img
      v-if="gradients" v-for="gradient in gradients"
      :src="gradient.gradient" :key="gradient.gradient"
      class="aspect-ratio-square rounded max-w-15 cursor-pointer"
      @click="setOpengraphID(gradient.id)" />
  </div>
</template>
