<template>
  <section class="w-full h-full py-4">
...
const filteredGradients = computed(() => {
	if (props.isSlider) {
		return gradientsWithStyles.value.slice(0, 3);
	} else {
		return gradientsWithStyles.value.filter(
			(gradient) => gradient.id === props.selectedId,
		);
	}
});
